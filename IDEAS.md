...
|------|------|--------|---------------|
| 2026-07-03 | **Dashboards + bulk actions via views/partials** (drop the config DSLs) — a design proposal grounding both features in CafeCar's existing view-override convention: a dashboard is one host-authored `cafe_car/dashboard/show` template composing `metric`/`chart` helpers; a bulk action decomposes into a `_bulk_actions` partial button + a model bang method + a policy predicate. Reworks the mechanism, keeps the features. | proposed | Owner-directed (7/3 — "no config DSLs; configure via views and partials"). Design note `docs/design/dsl-to-views-partials.md` (commit 3e517d9), emailed to owner for sign-off before build (I promised a preview since it sets the convention). Reworks blocked-on-owner until he answers the 2 taste Qs (whitelist home, template path). |
